...
`MachineLearning/Supervised/KdTree/KdTreeSimulation` owns Phase 1 step 5. Feature points and the query remain project-owned `ManualVector` values and Euclidean point distance is reused through `VectorSimulation`. KD-Tree owns explicit node records, alternating split axes, median-range ordering, child links, nearest-side descent, backtracking, split-plane comparison and subtree pruning. The current teaching build manually merge-sorts each recursive active range, so its documented build complexity is `O(n log² n)` rather than silently claiming a stronger construction algorithm.
...
`MachineLearning/Unsupervised/KMeans/KMeansSimulation` owns Phase 1 step 6. Feature points and centroids are project-owned `ManualVector` values, while Euclidean point-to-centroid distance is reused through `VectorSimulation`. K-Means owns the explicit nearest-centroid scan, assignment and distance arrays, per-cluster sum/count mean update, centroid movement, empty-cluster behavior, inertia, and convergence checks.

The live `/ml-foundations/k-means` Client intentionally renders 2D geometry but Core remains dimension-independent. The page follows the common Build → Predict → Watch → Visual/Memory → decision-guide → Practice contract, persists practice evidence through the existing C# state path, and uses the shared Last Run modal overlay. Decision Tree is Phase 1 step 7; PCA is Phase 1 step 8 and closes Phase 1. Phase 2 is complete with MLP (9), Backpropagation (10), and SGD / Momentum / Adam (11).
...
Every live Machine Learning lesson uses `ml-module-page` on the page root and `ml-state-surface` as the ordinary Visual/Memory card, but **`ml-state-surface` is not the scroll container**. The actual spatial drawing lives inside `ml-visual-board` (or an equivalent component-local board): a bounded viewport with native `overflow:auto`, overscroll containment, and a content-bounded stage. Headers, legends, Current Step, compact metrics, and Memory explanations remain outside that board. Algorithm-specific SVG stages preserve their `viewBox` aspect ratio and use `max-width:none`; they may grow only when real geometry/density requires more room, so scroll range tracks occupied content rather than a permanently oversized canvas. KNN, KD-Tree, K-Means, Gradient Descent, Linear/Logistic Regression, Decision Tree, PCA, MLP, Backpropagation, and optimizer visualizations all follow this boundary.
...
## Decision Tree Phase 1 step 7

`AlgorithmVisualizer.Core.MachineLearning.Supervised.DecisionTree` adds a project-owned binary classification tree. Training features are copied into `ManualVector` objects; labels remain a compact `int[]`. Tree nodes are custom records/classes, not framework tree nodes. For every active node the simulation explicitly insertion-sorts example indexes by each feature, generates thresholds between distinct adjacent values, partitions indexes into left/right arrays, scores Gini or entropy impurity reduction, commits the best gain, and recursively grows child nodes until a pure node or a stopping rule is reached. Prediction traverses the resulting child IDs in `O(depth)`. The Core project uses the SDK default recursive `Compile` glob, so these nested source files are discovered automatically without duplicate include/remove entries.
...
`MachineLearning/Unsupervised/Pca/PcaSimulation` owns Phase 1 step 8. Input rows are copied into `ManualVector` values; centered rows, the mean vector, dominant principal component, and projected reconstruction rows use the same project-owned storage. Core explicitly computes feature means, sample covariance with nested loops, covariance-by-vector products, a normalized power-iteration sequence for the dominant eigenvector, the corresponding eigenvalue and explained-variance ratio, and one scalar coordinate per example. It does not call a PCA/eigen/numerical package or `System.Numerics.Vector<T>`. The Core project relies on the SDK default recursive `Compile` glob, so these nested source files are included automatically.

The live `/ml-foundations/pca` page intentionally visualizes two features while Core remains dimension-independent and has non-2D test coverage. The Visual State keeps the lesson card static and places original points, mean, PC1 axis, projection links, and projected positions inside the shared scrollable visualization board as one dominant story. Memory State exposes raw/centered/projected rows, covariance cells, PC1, and scalar projections. Default controls are preset → dominant-direction prediction → Run; raw dataset editing is progressive disclosure. Neuron + Activation Functions is concept-only; MLP is the live Phase 2 step 9 and is the first deep-learning visualization.



## Deep Learning Phase 2 boundary

Phase 2 deliberately has no standalone Computational Graph module. Dependency/value-flow ideas are represented only inside concrete MLP and Backpropagation computations so the learner always sees what a node or edge means for a model parameter or activation.

`MachineLearning/DeepLearning/Common/ActivationMath` owns project-written scalar activations and derivatives and remains shared infrastructure rather than a standalone lesson. `Mlp/MlpSimulation` stores one dense hidden layer in `ManualMatrix`, hidden/output vectors in `ManualVector`, and performs the input → hidden → output forward pass with explicit loops. `Backpropagation/BackpropagationSimulation` reuses a matching network shape, retains forward values, computes output delta, output gradients, hidden deltas, hidden gradients, and one parameter update with the chain rule. `Optimizers/OptimizerSimulation` compares SGD, Momentum, and Adam on one deterministic cyclic sample stream, keeping first/second-moment state explicitly.

Deep Learning Core files are included by the SDK default recursive `Compile` glob; the project file does not duplicate those compile items. Client routes are `/deep-learning/mlp`, `/deep-learning/backpropagation`, and `/deep-learning/optimizers`. Their visualization components use the shared ML board-only scroll contract; the surrounding lesson state surface does not scroll.


## Graph ML Phase 3 boundary

Phase 3 is complete with four live modules: Sparse Matrix (step 12), PageRank (13), Spectral Clustering (14), and Message Passing / basic GNN (15). `MachineLearning/GraphMl/Common/ManualCsrMatrix` is the shared sparse-storage primitive. It keeps plain CSR arrays and explicit row traversal/SpMV rather than delegating to a sparse-matrix framework. `SparseMatrixSimulation` teaches dense-to-CSR conversion and SpMV, and its Client visual keeps dense cells, aligned CSR arrays, and the multiply result on one content-bounded board.

`PageRankSimulation` interprets CSR rows as directed outgoing adjacency and iterates a rank vector with explicit teleport mass, dangling-mass redistribution, source-by-source contributions, normalization, and L1 convergence. `SpectralClusteringSimulation` constructs a normalized Laplacian with `ManualMatrix`, calls the project-owned `ManualSymmetricEigenSolver` (Jacobi rotations), builds a row-normalized low-eigenvector embedding, and then reuses the existing manual `KMeansSimulation`. The dense Laplacian/Jacobi path is intentionally limited to small teaching graphs.

`MessagePassingSimulation` uses CSR neighborhoods plus project-owned feature arrays. Each layer gathers previous-layer neighbor features, mean/sum aggregates, applies separate shared self and neighbor weight matrices plus bias, runs ReLU, stages the new embedding, and commits all nodes together. The two-buffer design is an architectural invariant: in-place node updates during one layer are forbidden because they would make results depend on iteration order.

All Phase 3 visuals follow the ML board-only scroll contract: the sparse/graph/embedding drawing surface may overflow and scroll inside `.ml-visual-board`, while playback, Current Step, compact metrics, Memory State, decision guide, practice, and modal explanations stay in the normal lesson flow. Phase 3 Core files are included by the SDK default recursive `Compile` glob; no patch-specific compile-item overrides are required.
