...
The live `/ml-foundations/k-means` Client intentionally renders 2D geometry but Core remains dimension-independent. The page follows the common Build → Predict → Watch → Visual/Memory → decision-guide → Practice contract, persists practice evidence through the existing C# state path, and uses the shared Last Run modal overlay. Decision Tree is Phase 1 step 8; PCA is Phase 1 step 9 and closes Phase 1. Phase 2 is complete with Neuron + Activation Functions (10), MLP (11), Backpropagation (12), and SGD / Momentum / Adam (13).
...
All Phase 3 visuals follow the ML board-only scroll contract: the sparse/graph/embedding drawing surface may overflow and scroll inside `.ml-visual-board`, while playback, Current Step, compact metrics, Memory State, decision guide, practice, and modal explanations stay in the normal lesson flow. Nested Phase 3 Core files are explicitly included in `AlgorithmVisualizer.Core.csproj` so incremental patch ZIP application cannot silently omit the new namespaces.
