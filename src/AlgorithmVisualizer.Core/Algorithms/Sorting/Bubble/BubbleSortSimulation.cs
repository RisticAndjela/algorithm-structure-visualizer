...
/// teaching elements. Adjacent comparisons, swaps, pass boundaries, and the
/// optimized no-swap early exit are exposed as semantic simulation steps.
/// </summary>
public sealed class BubbleSortSimulation : SimulationAlgorithmBase
{
    ...
    public async Task<BubbleSortResult> SortAsync(CancellationToken cancellationToken = default)
    {
        ...
        await NextStepAsync(
            $"Start with {_elements.Length} values. Bubble Sort scans adjacent pairs from left to right; after each pass, one largest remaining value is fixed at the right.",
            cancellationToken);
