...
K-Means is live as Phase 1 step 7, Decision Tree as step 8, and PCA as the live Phase 1 step 9. Phase 1 is therefore complete through PCA; Computational Graph is the next planned lesson and begins Phase 2 at step 10.
...
All live ML Visual/Memory surfaces now use the shared internally scrollable canvas contract (`ml-module-page` + `ml-state-surface`), so wide feature plots, trees, and memory tables scroll inside the lesson surface instead of growing or clipping the whole page. KNN uses a bounded coordinate plot, KD-Tree removes stretched dead space before Current Step, and K-Means gives Feature Space the full-width dominant plot with visible points, centroids, assignments, and summaries below it.
...
The learner-facing lesson stays 2D so the geometry is visible: original points, the mean, the principal axis, projection links, and reconstructed positions on PC1 occupy one dominant scrollable feature-space canvas. Memory State exposes the raw/centered/projected vectors, mean, covariance cells, PC1, and scalar coordinates. Default controls are preset → direction prediction → Run PCA; raw points remain behind progressive disclosure. Guided Practice checks diagonal/horizontal/vertical dominant directions, zero-centered transformed data, and a case where one component preserves too little variance. Computational Graph is the next planned roadmap step 10 and begins Phase 2.
