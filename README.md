...
- Sparse Matrix (CSR storage and sparse matrix-vector multiplication)
- PageRank (sparse directed rank propagation)
- Spectral Clustering (Laplacian embedding + K-Means)
- Message Passing / basic GNN (synchronous graph feature aggregation)
...
All sorting pages now use the same learner-facing design language as the mature data-structure modules instead of a separate sorting-specific landing-page style. Live sorting labs opt into `learning-module-page learning-module-shell`; their **LEARN FIRST** area uses the shared explanation + 2×2 concept-card pattern, followed by the same compact module-header, smart-tip, bordered lab panels, compact Visual/Memory switch with an adjacent explanation, Guided Practice styling, exact Concepts & Memory links, and reusable Next Lesson navigation. Quick Sort and Heap Sort are both live labs using the shared learning shell. Future lessons should reuse the same live learning-shell primitives instead of keeping an unused placeholder component in the production client.
...
All currently planned base data-structure modules in the specification are represented by live labs; traversal/path algorithms remain separate.
...
K-Means is live as Machine Learning step 6, Decision Tree as step 7, and PCA as step 8. Phase 1 is complete through PCA. Phase 2 is also complete: Neural Network / MLP is step 9, Backpropagation step 10, and SGD / Momentum / Adam step 11. Phase 3 is complete: Sparse Matrix is step 12, PageRank step 13, Spectral Clustering step 14, and Message Passing / basic GNN step 15.
...
The learner-facing lesson stays 2D so the geometry is visible: original points, the mean, the principal axis, projection links, and reconstructed positions on PC1 occupy one responsive feature-space surface without a nested Visual State scroller. Memory State exposes the raw/centered/projected vectors, mean, covariance cells, PC1, and scalar coordinates. Default controls are preset → direction prediction → Run PCA; raw points remain behind progressive disclosure. Guided Practice checks diagonal/horizontal/vertical dominant directions, zero-centered transformed data, and a case where one component preserves too little variance. Neural Network / MLP is Machine Learning step 9. Weighted-sum and activation-function basics remain available as concept material and are shown where they matter inside the network.


### Deep Learning — Phase 2 steps 9–12

Phase 2 starts directly with `/deep-learning/mlp`, because the weighted-sum + activation calculation is too small to justify a standalone visualization. The neuron formula and activation choices remain concept material and are visible inside the MLP itself. `/deep-learning/mlp` builds one hidden layer and one output neuron with project-owned `ManualVector`/`ManualMatrix` storage. `/deep-learning/backpropagation` reuses the same small network, stores forward values, applies the chain rule backward, exposes output/hidden deltas and every parameter gradient, then performs one visible learning update. `/deep-learning/optimizers` feeds the same deterministic cyclic sample stream to SGD, Momentum, and Adam so their different state and parameter paths can be compared directly.

The learner UI keeps the approved shell and the no-nested-scroll Visual State contract: network/parameter-space drawings resize or reflow with the lesson surface; playback, Current Step, compact metrics, Memory State, decision guide, practice, and Last Run modal stay in normal page flow. Computational Graph and Neuron + Activation Functions are deliberately absent as standalone modules; those ideas are kept as concepts and taught inside concrete MLP/Backpropagation computations. Phase 3 is also complete: Sparse Matrix (12) → PageRank (13) → Spectral Clustering (14) → Message Passing / basic GNN (15).
...
All four Phase 3 lessons follow the approved shared shell and the no-nested-scroll Visual State contract. Core does not call sparse-matrix, PageRank, eigensolver, spectral-clustering, graph-neural-network, tensor, or automatic-differentiation libraries. The final Message Passing lesson closes the current Machine Learning curriculum at step 15.
...
### Machine Learning Visual State fit-to-frame pass (Aug 23, 2026)

All Machine Learning lessons now treat the **rendered footprint**, not only the mathematical point coordinate, as part of the viewport contract. Linear/Logistic Regression, KNN, KD-Tree, K-Means, Decision Tree, PCA, optimizer paths, PageRank, Spectral Clustering, and Message Passing reserve an inner safety inset for markers/halos/labels and fit the full active state into the visible surface. Fixed teaching canvases that previously depended on widths such as 920px/1120px/1010px/46rem were removed or converted to responsive coordinate frames. PageRank, Spectral Clustering, and Message Passing keep their graph geometry aligned with responsive SVG view boxes while their HTML node cards use percentage positions from the same coordinate system.

Deep-learning flow visuals (MLP and Backpropagation) and Sparse Matrix no longer rely on a hidden fixed-width strip. They reflow at explicit breakpoints; only genuinely long CSR/vector arrays may scroll locally. The outer ML Visual State remains non-scrolling. This pass is intentionally visible: shrinking the lesson width should reflow or scale the complete teaching state rather than crop it or require the learner to discover off-screen content.

### ML visual-state visibility follow-up
- K-Means and PCA coordinate plots use high-contrast, labeled point markers and explicit centroid/mean markers so the simulated geometry is immediately visible rather than merely within mathematical bounds.
- Spectral Clustering's Original Graph uses the same interaction model as the core Graph visualizer: C# pointer dragging, edges that follow node positions, resettable manual layout, and an inner canvas that expands/scrolls only when graph content reaches beyond the initial viewport.
