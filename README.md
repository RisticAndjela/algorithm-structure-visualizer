...
Sorting run workspaces now follow one fixed vertical hierarchy across Bubble, Selection, Merge, Quick, Heap, and advanced Topological Sort: **run action + compact `SimulationToolbar` → live metrics/current-role/invariant context → Visual/Memory state → shared color key → reference links**. The shared `StateLegend` owns legend spacing/colors instead of each visualization inventing its own legend chrome, and `SimulationToolbar.Compact` reduces playback/current-step/step-delay density only for these dense algorithm labs. Algorithm visualization logic, code snippets, semantic frames, and Memory State implementations remain algorithm-specific.
...
Gradient Descent is the first live Machine Learning module and Phase 1 roadmap step 2; Vector remains roadmap step 1 but lives under Data Structures. It minimizes the transparent convex objective `J(theta) = 1/2 * Σ curvature[i] * (theta[i] - target[i])²`, which keeps the optimization mechanics visible before a supervised model is introduced. The objective-specific loss and analytical gradient use explicit C# loops. The optimizer then **reuses the existing `VectorSimulation`** for L2 gradient norm, `eta * gradient`, and `theta - eta*gradient`, so the ML layer does not duplicate Vector arithmetic.
...
K-Nearest Neighbors is the fourth live Machine Learning lesson and Phase 1 roadmap step 5. The learner-facing visual is intentionally two-dimensional: labeled examples are stored, a new query point is placed in the same feature space, every distance is measured, the nearest `k` examples are retained, and their labels vote. Core remains dimension-independent and is tested beyond 2D.
...
KD-Tree is the fifth live Machine Learning lesson and Phase 1 roadmap step 6. It keeps the learner-facing geometry two-dimensional while Core remains dimension-independent. The build alternates coordinate axes by depth, explicitly merge-sorts the active point-index range on that axis, chooses the median point, and stores explicit left/right node links. This teaching build is `O(n log² n)`; it intentionally favors transparent median construction over hiding selection behind a library.
...
The beginner UI starts with one preset dataset and Gini impurity. Entropy, max depth, and raw `x,y,label` editing are progressively disclosed. Prediction asks which feature the root question will use. Visual State shows labeled 2D examples, the current/committed threshold, and the tree node hierarchy; Memory State shows fixed examples beside custom node records (example indexes, class counts, impurity, split feature/threshold/gain, child IDs, and leaf prediction). Guided Practice validates both root features, a deeper tree, an already-pure root leaf, and an imperfect/noisy case. The shared Last Run modal explains the chosen root rule, depth, leaf count, and training accuracy. PCA is the live Phase 1 step 9 that follows Decision Tree.

All live ML lessons now keep the **lesson state surface itself static** and put scrolling only on the drawing board. Coordinate/graph components use the shared `ml-visual-board` contract: a bounded viewport with native two-axis scrolling around a content-bounded stage, while headers, legends, Current Step, metrics, and Memory explanations stay outside the scroll area. This matches the approved Graph workspace behavior and prevents the old failure mode where an entire Visual/Memory card scrolled as one giant panel. SVG plots preserve their viewBox aspect ratio instead of being stretched into wide fixed-height containers, so KNN, KD-Tree, K-Means, Gradient Descent, regression plots, Decision Tree, PCA, and the Deep Learning boards keep their points/model geometry visible and inspectable.
...
The learner-facing lesson stays 2D so the geometry is visible: original points, the mean, the principal axis, projection links, and reconstructed positions on PC1 occupy one dominant scrollable feature-space canvas. Memory State exposes the raw/centered/projected vectors, mean, covariance cells, PC1, and scalar coordinates. Default controls are preset → direction prediction → Run PCA; raw points remain behind progressive disclosure. Guided Practice checks diagonal/horizontal/vertical dominant directions, zero-centered transformed data, and a case where one component preserves too little variance. Neuron + Activation Functions is live as Phase 2 step 10 and leads into MLP, Backpropagation, and optimizer comparison.


### Deep Learning — Phase 2 steps 10–13
...
- `RunMetricStrip` is the shared live-metric component for sorting/ordering lessons. Do not recreate raw metric rows locally; each metric keeps a compact bordered card so moving it between components cannot drop its visual design.
