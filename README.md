...
Deep-learning flow visuals (Neuron, MLP, Backpropagation) and Sparse Matrix no longer rely on a hidden fixed-width strip. They reflow at explicit breakpoints; only genuinely long CSR/vector arrays may scroll locally. The outer ML Visual State remains non-scrolling. This pass is intentionally visible: shrinking the lesson width should reflow or scale the complete teaching state rather than crop it or require the learner to discover off-screen content.
